...
@api_view(['GET', 'PUT', 'DELETE'])
def Products_detail(request, id, format=None):
        try:
...
def Sales_detail(request, id, format=None):
        ...
        if request.method == 'GET':
          serializer = ProductsSerializer(sales)
          return Response(serializer.data)
        elif request.method == 'PUT':
          serializer = ProductsSerializer(sales, data=request.data)
          if serializer.is_valid():
             serializer.save()
             return Response(serializer.data)
          return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
@api_view(['GET', 'POST'])
def salesItems_list(request, format=None):

    if request.method =='GET':
     Sales = salesItems.objects.all()
     serializer = salesItemsSerializer(Sales, many=True)
     return Response(serializer.data)
    
    if request.method =='POST':
      serializer = salesItemsSerializer(data=request.data)
      if serializer.is_valid():
       serializer.save()
...
@api_view(['GET', 'PUT', 'DELETE'])
def salesItems_detail(request, id, format=None):
        try:
           
           sales = salesItems.objects.get(pk=id)
        except salesItems.DoesNotExist:
           return Response(status=status.HTTP_404_NOT_FOUND)
        
        
        if request.method == 'GET':
          serializer = salesItemsSerializer(sales)
          return Response(serializer.data)
        elif request.method == 'PUT':
          serializer = salesItemsSerializer(sales, data=request.data)
          if serializer.is_valid():
             serializer.save()
             return Response(serializer.data)
          return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
